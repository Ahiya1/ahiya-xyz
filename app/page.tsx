        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {projects.map((project) => (
              <div
                key={project.id}
                className={`group relative bg-white/5 backdrop-blur-xl border ${project.borderColor} rounded-2xl p-6 hover:bg-white/10 transition-all duration-500 cursor-pointer hover:scale-[1.02] hover:${project.glowColor} hover:shadow-2xl`}
                onMouseEnter={() => setActiveProject(project.id)}
                onMouseLeave={() => setActiveProject(null)}
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`p-3 rounded-xl bg-gradient-to-r ${project.theme} text-white shadow-lg`}
                  >
                    {project.icon}
                  </div>
                ...
                {/* Hover glow effect */}
                <div
                  className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${project.theme} opacity-0 group-hover:opacity-5 transition-opacity duration-500 pointer-events-none`}
                />
              </div>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {writings.map((writing) => (
              <div
                key={writing.id}
                className="group relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all duration-500 cursor-pointer hover:scale-[1.02] hover:shadow-2xl"
              >
                <div className="mb-4">
                  <div
                    className={`inline-flex items-center px-3 py-1 rounded-full bg-gradient-to-r ${writing.theme} bg-opacity-20 border border-current/30 mb-4`}
                  >
                ...
                {/* Hover glow effect */}
                <div
                  className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${writing.theme} opacity-0 group-hover:opacity-5 transition-opacity duration-500 pointer-events-none`}
                />
              </div>
            ))}
          </div>
